...
def polytr(polyorder,X,Y):
     ...
     d=d/25
     #annoted when cv
     #xt=np.linspace(-1, 1.2, 200)
     #yt=np.linspace(1, 2, 200)
     #for i in range(200):
          ##yt[i]=random.gauss(np.dot(np.transpose(w),np.reshape(phi2(xt[i],polyorder),(polyorder+1,1)))[0],d)
          #yt[i]=np.dot(np.transpose(w),np.reshape(phi2(xt[i],polyorder),(polyorder+1,1)))[0]
     #plt.plot(xt,yt,label=s)
     return w,d
...
def cvintr(cvorder):
     ...
     d=0
     for i in range(N):
          #x_train, x_test, y_train, y_test=train_test_split(X, Y, test_size=0.04,random_state=i)
...
#ridge(20,0.00000001,0.1,"over-fitting with lamda=0.00000001")
#ridge(20,10.0,0.1,"under fitting with lamda=10")
